...
public class BuyController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getSession().getAttribute("user") == null) {
            ...
            session.setAttribute("loginInvalid", "Khách hàng vui lòng đăng nhập trước khi tiến hành mua !!");
            response.sendRedirect("Cart");
        } else {
            Account user = (Account) request.getSession().getAttribute("user");
            HttpSession session = request.getSession();
            if (user.getRole().equalsIgnoreCase("admin")) {
                session.setAttribute("notiBuy", "Bạn đang đăng nhập dưới quyền Admin! không thể đặt hàng");
            ...
            session.setAttribute("notiBuy", "Đặt hàng thành công!!<br> Quý khách có thể kiểm tra lại ở mục lịch sử mua hàng");
            session.removeAttribute("cart");
            session.removeAttribute("totalPrice");
            session.removeAttribute("cartSize");
            response.sendRedirect("Store");
